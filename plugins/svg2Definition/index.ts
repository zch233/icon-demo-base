...
function element2AbstractNode({
  ...
  return ({ name: tag, attributes, children }: XmlElement): AbstractNode => {
    const data = {
      tag,
      attrs: {...attributes},
      children: (children as XmlElement[]).filter(({type}) => type === 'element').map(item => element2AbstractNode({
        name,
        theme,
        extraNodeTransformFactories
      })(item)),
    ...
    return extraNodeTransformFactories.map((factory: TransformFactory) => factory({ name, theme }))
